...
def create_app(environment: str = "development") -> Flask:
    ...
    from worker.utils.url_converters import (ListConverter,
                                             IntListConverter)
...
def register_blueprints(_app: Flask) -> None:
    ...
    Args:
        _app: Address of :flask:`Flask` application instance.
    """
    from worker.bps.index import bp_index
    _app.register_blueprint(bp_index)

    from worker.bps.api import bp_api
    _app.register_blueprint(bp_api)

    from worker.bps.util import bp_util

...
class MarmaraUniversityAuthorsSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for author_section in author_sections:
            requests.post(f"{self.operator_url}/univerdustry/add/author", json={"author_name": self.get_author_name(author_section)})
        ...
        if next_page_button.attrib.get('disabled', '') != 'disabled':
            next_page_url = self.paginator_url.format(domain=response.meta["university_domain"], 
                                                      last_author_id=next_page_button.attrib["onclick"].split('\\')[-3][3:], 
